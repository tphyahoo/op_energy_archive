...
class HTTP_server:
    ...
    ##    idBeg=200000&span=2016&idEnd=360000&mime_type=csv

    #url_pattern = re.compile( '^((.*)\?idA=([0-9]+)&idB=([0-9]+))', re.IGNORECASE)
    url_pattern = re.compile( '^((.*)\?idBeg=([0-9]+)&span=([0-9]+)&idEnd=([0-9]+))', re.IGNORECASE)
        ...
        if True:
           ...
           if path[-4:] == '.cgi':
                if arg_string:
                    ...
                    args = [ components.group(3), components.group(4), components.group(5) ]
    ...
    def write_log(self, in_text):
        with open('server/log.txt', 'a') as f:
            f.write("LOG:" + str(datetime.now())+' - '+in_text+'\n')
