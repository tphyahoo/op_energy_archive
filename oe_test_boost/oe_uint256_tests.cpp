...
//  adopted to local test  14nov20 -dbb


//define BOOST_TEST_MODULE oe_uint256_test
...
#include "test_oe_bitcoin.h"

//BOOST_FIXTURE_TEST_SUITE(uint256_tests, BasicTestingSetup)
BOOST_FIXTURE_TEST_SUITE(uint256_tests, oe_fixture)
...
BOOST_AUTO_TEST_CASE( basics ) // constructors, equality, inequality
{
    ...
    std::cout << "TEST_CASE( basics ) \n";
    std::cout << "  R1L.ToString() 0x" << R1L.ToString() << "\n";
    std::cout << "  R1S.ToString() 0x" << R1S.ToString() << "\n";
    std::cout << "  R2L.ToString() 0x" << R2L.ToString() << "\n";
    std::cout << "  R2S.ToString() 0x" << R2S.ToString() << "\n";

    std::cout << "  ZeroL.ToString() 0x" << ZeroL.ToString() << "\n";
    std::cout << "  ZeroS.ToString() 0x" << ZeroS.ToString() << "\n";
    std::cout << "  MaxL.ToString()  0x" << MaxL.ToString() << "\n";
    std::cout << "  MaxS.ToString()  0x" << MaxS.ToString() << "\n";
    std::cout << "  OneL.ToString()  0x" << OneL.ToString() << "\n";
    std::cout << "  OneS.ToString()  0x" << OneS.ToString() << "\n";
...
BOOST_AUTO_TEST_CASE( comparison ) // <= >= < >
{
    ...
    std::cout << "TEST_CASE( comparison ) // <= >= < >" << "\n";
    bool tResB;

    tResB = (ZeroL < R1L);
    std::cout << " (ZeroL < R1L) = " << tResB << "\n";
    BOOST_CHECK( ZeroL < R1L );
    tResB = (R2L < R1L);
    std::cout << " (R2L < R1L) = " << tResB << "\n";
    BOOST_CHECK( R2L < R1L );
    tResB = (ZeroL < OneL);
    std::cout << " (ZeroL < OneL) = " << tResB << "\n";
    BOOST_CHECK( ZeroL < OneL );
    tResB = (OneL < MaxL);
    std::cout << " (OneL < MaxL) = " << tResB << "\n";
    BOOST_CHECK( OneL < MaxL );
    tResB = (R1L < MaxL);
    std::cout << " (R1L < MaxL) = " << tResB << "\n";
    BOOST_CHECK( R1L < MaxL );
    tResB = (R2L < MaxL);
    std::cout << " (R2L < MaxL) = " << tResB << "\n";
...
{
    std::cout << "TEST_CASE( conversion ) // \n";
    std::cout << " ArithToUint256(UintToArith256(ZeroL)) \n   0x" << ZeroL.ToString() << "\n";
    std::cout << " ArithToUint256(UintToArith256(OneL))  \n   0x" << OneL.ToString() << "\n";
    std::cout << " ArithToUint256(UintToArith256(R1L))   \n   0x" << R1L.ToString() << "\n";
    std::cout << " ArithToUint256(UintToArith256(R2L))   \n   0x" << R2L.ToString() << "\n";
    std::cout << " arith_uint256(R1L.GetHex())           \n   0x" << arith_uint256(R1L.GetHex()).ToString() << "\n";
...
BOOST_AUTO_TEST_CASE( add_mem )
{
    unsigned char bufp[ 32 ];
    //GetStrongRandBytes( bufp, 32 );
    //arith_uint256 v = UintToArith256( &bufp );
    //std::cout << "  GetStrongRandBytes() 0x" << v.ToString()
    BOOST_CHECK( bufp != NULL );
}
